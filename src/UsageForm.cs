        ...
        async Task OpenLoginAsync(ServiceState service)
        {
            ...
            if (useCli)
            {
                if (!IsCliAvailable(cliName))
                {
                    ...
                    Invalidate();
                    MessageBox.Show(
                        T("CLI topilmadi. Sozlamalarda login usulini Browser OAuth qilib o'zgartiring.",
                ...
                string title  = T("Headroom: login tugagach bu oynani yopishingiz mumkin",
                                  "Headroom: close this window after sign-in");
                string banner = service.Name == "Claude"
                    ? T("[Headroom] Login uchun pastga /login yozing. Tugagach bu oynani yopishingiz mumkin.",
                        "[Headroom] Type /login below to sign in. You can close this window once login completes.")
                    : T("[Headroom] Browserda login qiling. Tugagach bu oynani yopishingiz mumkin.",
                ...
                {
                    MessageBox.Show(T("CLI ishga tushmadi: ", "Failed to launch CLI: ") + ex.Message,
