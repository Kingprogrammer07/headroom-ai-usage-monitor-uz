...
namespace Headroom
{
    partial class UsageForm
    {
        ...
        {
            if (key == "close") return T("Yopish", "Close");
            if (key == "pin") return T(settings.AlwaysOnTop ? "Tepada turishni o'chirish" : "Har doim tepada", settings.AlwaysOnTop ? "Unpin from top" : "Always on top");
            if (key == "token") return T(settings.ClaudeShowUsed ? "Qolganini ko'rsatish" : "Ishlatilganni ko'rsatish", settings.ClaudeShowUsed ? "Switch to remaining" : "Switch to used");
            if (key == "fiveReset") return T(string.Equals(settings.FiveHourResetMode, "relative", StringComparison.OrdinalIgnoreCase) ? "5 soat reset: countdown -> soat" : "5 soat reset: soat -> countdown", string.Equals(settings.FiveHourResetMode, "relative", StringComparison.OrdinalIgnoreCase) ? "5h reset: countdown → clock time" : "5h reset: clock time → countdown");
            if (key == "weekReset") return T(string.Equals(settings.WeeklyResetMode, "relative", StringComparison.OrdinalIgnoreCase) ? "Haftalik reset: countdown -> soat" : "Haftalik reset: soat -> countdown", string.Equals(settings.WeeklyResetMode, "relative", StringComparison.OrdinalIgnoreCase) ? "Weekly reset: countdown → clock time" : "Weekly reset: clock time → countdown");
            if (key == "settings") return T("Sozlamalar", "Settings");
            if (key.EndsWith("-refresh")) return T("Yangilash", "Refresh");
            if (key.EndsWith("-boost"))
            {
                ServiceState svc = key.StartsWith("codex") ? codex : claude;
                return svc.BoostActive ? T("Boostni to'xtatish", "Stop boost") : T("Boost (tez-tez yangilash)", "Boost (frequent refresh)");
            }
            if (key.EndsWith("-login")) return T("Kirish", "Login");
            return "";
        }
