    ...
    partial class UsageForm
    {
        ...
        void DrawService(Graphics g, ServiceState state, int x, int y, int w, int h, string keyPrefix)
        {
            ...
            using (var dim = new SolidBrush(Color.FromArgb(185, 190, 205)))
            {
                ...
                int badgeX = x + 100;
                if (stale)
                    badgeX += DrawBadge(g, T("Eski", "Stale"), badgeX, y + 14, Color.FromArgb(110, 85, 20), Color.FromArgb(180, 150, 50)) + 6;
                if (exhausted)
                    badgeX += DrawBadge(g, T("Limit", "Limit"), badgeX, y + 14, Color.FromArgb(100, 35, 35), Color.FromArgb(220, 100, 100)) + 6;
                if (state.Status == "fetch_error")
                    badgeX += DrawBadge(g, T("Xato", "Error"), badgeX, y + 14, Color.FromArgb(90, 62, 28), Color.FromArgb(235, 170, 70)) + 6;
                if (state.Status == "rate_limited")
                    DrawBadge(g, T("Kutish", "Wait"), badgeX, y + 14, Color.FromArgb(80, 64, 28), Color.FromArgb(220, 185, 80));
                ...
                if (!state.Data.HasAnyValue())
                {
                    ...
                    string status = state.IsRefreshing ? T("Yangilanmoqda", "Updating") : StatusText(effectiveStatus);
                ...
                DrawRow(g, T("5s", "5h"), state.Data.FiveHourDisplayPercent(showUsed), state.DisplayedFivePct, showUsed, false, state.Data.FiveHourReset, state.Data.FiveHourNotStarted, settings.FiveHourResetMode, x, firstY, w, fiveAccent, label, reset, num, white, muted, dim, keyPrefix + "-fiveMode", keyPrefix + "-fiveResetLabel", fiveLockedByWeekly, fiveResetTracking);
                DrawRow(g, T("1h", "1W"), state.Data.WeeklyDisplayPercent(showUsed), state.DisplayedWeekPct, showUsed, true, state.Data.WeeklyReset, state.Data.WeeklyNotStarted, settings.WeeklyResetMode, x, secondY, w, state.Accent, label, reset, num, white, muted, dim, keyPrefix + "-weekMode", keyPrefix + "-weekResetLabel", false, weekResetTracking);
            }
        }
        ...
        string StatusText(string status)
        {
            switch (status)
            {
                case "updating": return T("Yangilanmoqda", "Updating");
                case "rate_limited": return T("429 kutish", "Rate limited");
                case "fetch_error": return T("API vaqtincha ishlamayapti", "Temporarily unreachable");
                case "no_data": return T("Ma'lumot yo'q", "No data");
                case "login_required": return T("Login qiling", "Please log in");
                case "login_pending": return T("Login ketmoqda... browserda tasdiqlang", "Signing in… complete it in your browser");
                case "fixture_missing": return T("Fixture yo'q", "Fixture missing");
                case "no_usage_text": return T("Usage matni yo'q", "No usage text");
                case "starting": return T("Ishga tushmoqda", "Starting");
                default: return status ?? T("Ma'lumot yo'q", "No data");
            }
        }
        ...
        string LimitResetText(UsageData data, int? fiveRemain, int? weekRemain, bool english)
        {
            ...
            return english ? text.Replace("Reset in ", "in ") : text.Replace("Resetgacha ", "");
        }
        ...
        string BoostText(ServiceState state)
        {
            ...
            return English ? min + "m left" : min + " daq qoldi";
        }
        ...
        void DrawRow(Graphics g, string label, int? pct, double? barPct, bool showUsed, bool weekly, string resetText, bool notStarted, string resetMode, int x, int y, int w, Color accent, Font labelFont, Font resetFont, Font numFont, Brush white, Brush muted, Brush dim, string hitMode = null, string hitReset = null, bool disabled = false, bool resetTracking = false)
        {
            ...
            {
                int labelX = x + 12;
                int label5hW = (int)Math.Ceiling(g.MeasureString(T("5s", "5h"), labelFont).Width);
                int labelWkW = (int)Math.Ceiling(g.MeasureString(T("1h", "1W"), labelFont).Width);
                int maxLabelW = Math.Max(label5hW, labelWkW);
                string modeText = showUsed ? T("Ish", "Used") : T("Qol", "Rem");
                int modeColW = Math.Max(
                    (int)Math.Ceiling(g.MeasureString(T("Ish", "Used"), labelFont).Width),
                    (int)Math.Ceiling(g.MeasureString(T("Qol", "Rem"), labelFont).Width));
                ...
                string reset = notStarted ? T("Boshlanmagan", "Not started") : ResetText(resetText, resetMode, English, weekly);
        ...
        static string ResetText(string raw, bool preferAbsolute, bool english, bool weekly)
        {
            ...
            if (cleaned.Contains("リセットまで")) return english ? cleaned.Replace("リセットまで", "Reset in") : cleaned.Replace("リセットまで", "Resetgacha");
            ...
            {
                if (preferAbsolute) return (english ? "Reset " : "Reset ") + FormatResetTime(target, english);
                return (english ? "Reset in " : "Resetgacha ") + FormatDuration(target - DateTime.Now, english);
            }
            ...
            if (cleaned.Contains("後にリセット"))
            {
                ...
                return english ? "Reset in " + TranslateDurationText(s) : "Resetgacha " + TranslateDurationText(s).Replace("h", "soat").Replace("m", "daq");
            }
            if (lower.Contains("reset"))
            {
                var m = Regex.Match(cleaned, @"(\d{1,2}/\d{1,2}\s+\d{1,2}:\d{2})");
                if (m.Success) return (english ? "Reset " : "Reset ") + m.Groups[1].Value;
                return cleaned;
            }
            return (english ? "Reset " : "Reset ") + cleaned;
        }
        ...
        static string RelativeResetText(string text, bool preferAbsolute, bool english, bool weekly)
        {
            ...
            if (preferAbsolute) return (english ? "Reset " : "Reset ") + FormatResetTime(DateTime.Now.Add(span), english);
            return (english ? "Reset in " : "Resetgacha ") + FormatDuration(span, english);
        }
        ...
        {
            if (span.TotalSeconds <= 0) return english ? "0m" : "0daq";
            ...
            if (days > 0) return days + "k " + hours + "s " + minutes + "daq";
            if (hours > 0) return hours + "s " + minutes + "daq";
            return minutes + "daq";
        }
        ...
        static string FormatResetTime(DateTime value, bool english)
        {
            ...
            if (value.Date == today.AddDays(1)) return (english ? "Tomorrow " : "Ertaga ") + value.ToString("H:mm");
        ...
        void DrawLoginButton(Graphics g, int x, int y, string key)
        {
            ...
            using (var f = new Font("Segoe UI", 9.2f, FontStyle.Bold))
                TextRenderer.DrawText(g, T("Kirish", "Login"), f,
        ...
        void DrawTokenToggleIcon(Graphics g, Rectangle r, Color color)
        {
            ...
            string text = showUsed ? T("I", "U") : T("Q", "R");
